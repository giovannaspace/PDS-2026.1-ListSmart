...
class Lista():
    ...
    @staticmethod
    def adicionarItem(idLista,nomeItem, quantidade, unidade, precoUnitario,observacoesItem,categorias,opcao1,opcao2,x,y,porcentagem):
        ...
        if opcao1 == 's':
            ...
            item = ItemPromocional(nomeItem, quantidade, unidade, precoUnitario,observacoesItem,categorias,idLista,desconto,0)

        elif opcao1 == 'n':
            item = ItemComum(nomeItem, quantidade, unidade, precoUnitario,observacoesItem,categorias,idLista,desconto,0)
    ...
    def itensDaLista(self):
        ...
        for linhas in linha:
            ...
            if linhas[5] == 0 or linhas[5] is None:
                item = ItemComum(linhas[1],linhas[2], linhas[3], linhas[4], linhas[8], linhas[9], linhas[7],linhas[5],linhas[6], linhas[0])

            else:
                 item = ItemPromocional(linhas[1],linhas[2], linhas[3], linhas[4], linhas[8], linhas[9], linhas[7],linhas[5],linhas[6], linhas[0])
    ...
    def calcularTetoOrcamentario(self):
        if self.__valorTotalMarcado > float(self.tetoOrcamentario):
            diferenca = abs(self.tetoOrcamentario - self.__valorTotalMarcado)
            return {"status" : "vermelho",
                    "mensagem" : f"Você ultrapassou o limite em R$ {diferenca:.2f}"
            }
        elif self.__valorTotalMarcado == float(self.tetoOrcamentario):
            return {
                "status" : "amarelo",
                "mensagem" : f"Você atingiu exatamente o limite de R$ {self.tetoOrcamentario :.2f}"
            }
        else:
            diferenca = abs(float(self.tetoOrcamentario) - self.__valorTotalMarcado)
            return {
                "status" : "verde",
